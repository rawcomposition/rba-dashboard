...
export default function Header({ title, parent }: Props) {
  ...
  return (
    <header className="bg-slate-900 h-[60px] shrink-0 flex items-center" onClick={close}>
      ...
      <Link href="/" className={clsx("sm:w-60 flex items-center flex-shrink-0", isSubPage && "hidden md:flex")}>
        <img src="/icon.png" className="w-[50px] mx-4" width="50" height="50" />
        <h1 className="text-center text-[#757c8c] font-logo text-2xl">RBA Dashboard</h1>
      </Link>
      {isSubPage && (
        <Link href="/" className="md:hidden pl-3 pr-5 py-3">
